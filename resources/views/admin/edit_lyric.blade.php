<x-layout>
    <x-slot:title>Edit Lyric</x-slot:title>

    <div class="max-w-3xl mx-auto py-10 px-6">
        <header class="mb-8 border-b border-gray-700 pb-4">
            <h1 class="text-3xl font-bold uppercase tracking-tighter">Edit Lyric</h1>
        </header>
        ...
        <form action="{{ route('admin.lyrics.update', $lyric->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="form-control w-full">
                ...
                <select name="single_id" class="select select-bordered rounded-none w-full" required>
                    ...
                    <option value="{{ $single->id }}"
                        {{ old('single_id', $lyric->single_id) == $single->id ? 'selected' : '' }}>
                        {{ $single->title }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-control w-full">
                ...
                <textarea name="lyrics_text" rows="15" class="textarea textarea-bordered rounded-none w-full" required
                    placeholder="Masukkan seluruh teks lirik dari lagu di sini...">{{ old('lyrics_text', $lyric->lyrics_text) }}</textarea>
                <label class="label">
            ...
            <div class="flex items-center gap-4 pt-6 border-t border-gray-800">
                <button type="submit" class="btn btn-primary rounded-none px-12 text-white">Update Lirik</button>
